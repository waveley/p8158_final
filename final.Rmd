...
>>>>>>> a2793a80979aa49ee84e4f3470b85fc0e323cd19
...
# Boxplot of Healthy Lifestyle
```{r}
healthy_lifestyle <- c("athlete_yn","hr_sleep", "smoking", "fruit_veg")

healthy_life_numeric <- dataset[,healthy_lifestyle] %>% mutate(
  hr_sleep = as.numeric(hr_sleep),
  athlete_yn = as.factor(athlete_yn),
  smoking = as.numeric(smoking)
)

ggplot(data = healthy_life_numeric, aes(y = hr_sleep, x = athlete_yn), group = athlete_yn) + geom_boxplot(aes(color = athlete_yn))+
  labs(title = "Hours of Sleep for Athletes and Non-athletes",
       y = "Hour of Sleep",
       x = " ") +
  scale_x_discrete(
    labels = c("Athletes", "Non-athletes")
...
ggplot(data = healthy_life_numeric, aes(x = smoking, fill = athlete_yn)) +
  geom_histogram(binwidth=1, color = "black",alpha=0.5) + 
  labs(title = "Frequency of Smoking Status",
       y = "Count",
       x = "Smoking Status")
```
